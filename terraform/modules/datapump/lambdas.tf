resource "aws_lambda_function" "dispatcher" {
  ...
  publish          = true
  tags             = local.tags
  layers           = [module.py38_datapump_021.layer_arn, var.rasterio_lambda_layer_arn]
  environment {
    variables = {
      ENV                           = var.environment
...
resource "aws_lambda_function" "executor" {
  ...
  publish          = true
  tags             = local.tags
  layers           = [module.py38_datapump_021.layer_arn]
  environment {
    variables = {
      ENV                            = var.environment
      S3_BUCKET_PIPELINE             = var.pipelines_bucket
...
resource "aws_lambda_function" "fastapi" {
  ...
  publish          = true
  tags             = local.tags
  layers           = [module.py38_datapump_021.layer_arn, var.lambda_layers.outputs.py38_fastapi_0670_arn]
  environment {
    variables = {
      ENV                            = var.environment
      DATA_API_URI                   = var.data_api_uri
