resource "aws_s3_bucket_object" "datapump_utils" {
  ...
  source = "${var.lambda_layers_path}/datapump_utils/layer.zip"
  etag   = filemd5("${var.lambda_layers_path}/datapump_utils/layer.zip")
}
...
resource "aws_lambda_layer_version" "datapump_utils" {
  ...
  source_code_hash    = filebase64sha256("${var.lambda_layers_path}/datapump_utils/layer.zip")
}
