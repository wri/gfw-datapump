...
def test_glad_landsat_sync(monkeypatch):
    ...
    today_month_day = today.strftime("%m_%d")
    today_version = "v" + today.strftime("%Y%m%d")
    ...
    def mock_get_gs_files(bucket, prefix, **kwargs) -> List[str]:
        ...
        if prefix == f"GLADalert/C2/{today.year}/final/alertDate22":
            return too_few
        elif (
            prefix == f"GLADalert/C2/{today.year}/{today.strftime('%m_%d')}/alertDate22"
        ):
            return just_right
        elif prefix == f"GLADalert/C2/{today.year - 1}/final/alertDate21":
            return just_right
        elif (
            prefix == f"GLADalert/C2/{today.year}/{today.strftime('%m_%d')}/alertDate21"
        ):
            return too_few
    ...
    job = raster_jobs[0]
    assert isinstance(job, RasterVersionUpdateJob)
    assert job.version == today_version
    assert job.content_date_range.max == today_date
    assert job.tile_set_parameters.source_uri == [
        f"gs://earthenginepartners-hansen/GLADalert/C2/{today.year - 1}/final/alert21*",
        f"gs://earthenginepartners-hansen/GLADalert/C2/{today.year - 1}/final/alertDate21*",
        f"gs://earthenginepartners-hansen/GLADalert/C2/{today.year}/{today_month_day}/alert22*",
        f"gs://earthenginepartners-hansen/GLADalert/C2/{today.year}/{today_month_day}/alertDate22*",
    ]
